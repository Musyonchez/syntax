...
import { apiClient, type PersonalSnippet, type CreatePersonalSnippetData } from '@/lib/api-client'

interface CreateSnippetModalProps {
  accessToken: string
  refreshToken: string
  onCreated: (snippet: PersonalSnippet) => void
  onClose: () => void
}
...
export function CreateSnippetModal({ accessToken, refreshToken, onCreated, onClose }: CreateSnippetModalProps) {
  const [formData, setFormData] = useState<CreatePersonalSnippetData>({
    title: '',
    description: '',
    code: '',
    language: 'javascript',
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const snippet = await apiClient.createPersonalSnippet(formData, accessToken, refreshToken)
      onCreated(snippet)
    } catch (err) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
      <div className="bg-background border border-border rounded-xl p-6 w-full max-w-2xl max-h-[90vh] overflow-y-auto m-4">
        <div className="flex items-center justify-between mb-6">
          <h2 className="text-xl font-semibold text-foreground">Create New Snippet</h2>
          <button
            onClick={onClose}
