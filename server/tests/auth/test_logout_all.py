#!/usr/bin/env python3
"""
Auth Test: Logout All Devices
Tests Phase 2 Priority 2 - Logout all devices functionality
"""

import asyncio
import aiohttp
import sys
import os

# Add shared modules to path
sys.path.append('../../shared')
sys.path.append('../../schemas')

from database import db

# Test configuration
AUTH_URL = "http://localhost:8081"
TEST_USER_EMAIL = "logout-test@example.com"
TEST_USER_NAME = "Logout All Test User"

async def test_logout_all_devices():
    """Test logout all devices functionality"""
    print("🧪 Testing Logout All Devices...")
    
    session = None
    try:
        session = aiohttp.ClientSession()
        await db.connect()
        
        # Clean any existing test data
        users_collection = await db.get_users_collection()
        tokens_collection = await db.get_refresh_tokens_collection()
        await users_collection.delete_many({"email": TEST_USER_EMAIL})
        await tokens_collection.delete_many({"userId": {"$regex": "logout-test"}})
        
        # 1. Create user and get access token
        login_data = {
            "email": TEST_USER_EMAIL,
            "name": TEST_USER_NAME,
            "avatar": "test-avatar.jpg"
        }
        
        access_token = None
        user_id = None
        
        async with session.post(f"{AUTH_URL}/google-auth", json=login_data) as resp:
            if resp.status == 200:
                result = await resp.json()
                data = result.get('data', {})
                access_token = data.get('token')
                user_id = data.get('user', {}).get('id')
            else:
                print(f"  ❌ Failed to create test user: {resp.status}")
                return False
        
        # 2. Create more sessions by logging in multiple times
        for i in range(2):
            async with session.post(f"{AUTH_URL}/google-auth", json=login_data) as resp:
                if resp.status != 200:
                    print(f"  ❌ Failed to create additional sessions: {resp.status}")
                    return False
        
        # 3. Verify we have tokens before logout
        tokens_before = await tokens_collection.count_documents({"userId": user_id})
        
        if tokens_before == 0:
            print("  ❌ No tokens found before logout")
            return False
        
        # 4. Call logout-all endpoint
        headers = {"Authorization": f"Bearer {access_token}"}
        async with session.post(f"{AUTH_URL}/logout-all", headers=headers) as resp:
            if resp.status == 200:
                result = await resp.json()
                
                # 5. Verify all tokens are gone
                tokens_after = await tokens_collection.count_documents({"userId": user_id})
                
                if tokens_after == 0:
                    revoked_count = result.get('data', {}).get('revokedTokens', 0)
                    print(f"  ✅ All tokens revoked. Count: {revoked_count}")
                    return True
                else:
                    print(f"  ❌ Tokens still exist: {tokens_after}")
                    return False
            else:
                error_text = await resp.text()
                print(f"  ❌ Request failed: {resp.status} - {error_text}")
                return False
                
    except Exception as e:
        print(f"  ❌ Exception: {e}")
        return False
    finally:
        if session:
            await session.close()
        # Cleanup test data
        try:
            await users_collection.delete_many({"email": TEST_USER_EMAIL})
            await tokens_collection.delete_many({"userId": user_id}) if user_id else None
        except:
            pass

async def main():
    """Run logout all devices test"""
    from dotenv import load_dotenv
    load_dotenv('../../.env')
    
    success = await test_logout_all_devices()
    return 0 if success else 1

if __name__ == "__main__":
    exit_code = asyncio.run(main())
    sys.exit(exit_code)